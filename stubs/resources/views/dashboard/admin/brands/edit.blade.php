    ...
    <div class="mb-5 card card-xxl-stretch mb-xl-8">
        ...
        <div class="pt-5 border-0 card-header">
            <h3 class="card-title align-items-start flex-column">
                <span class="mb-1 card-label fw-bolder fs-3">{{ $pageTitle . ' ' . $brand?->name}}</span>
            </h3>
        </div>
        <!--end::Header-->
            ...
            <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="tab-content" id="myTabContent">
                        @foreach(config('laravellocalization.supportedLocales') as $key => $lang)
                        <div class="tab-pane fade @if(app()->getLocale() == $key) show active @endif" id="{{ $key }}" role="tabpanel"
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <input type="text" id="{{ $key }}[name]" name="{{ $key }}[name]"
                                        value="{{ old($key . '.name', $brand->translate($key)->name ?? '') }}"
                                ...
                                <div class="col-md-8">
                                    ...
                                    <textarea name="{{ $key }}[description]" id="{{ $key }}[description]" cols="30" rows="10"
                                        class="form-control">{{ old($key . '.description', $brand->translate($key)->description ?? '') }}</textarea>
                                </div>
                            </div>
                        </div>
                        @endforeach
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                                            <div class="p-3 mb-3 text-center border rounded">
                                                ...
                                                <div class="mt-2">
                                                    <img id="brandPreview" src="{{ $brand->getMediaUrl('brand', $brand, null, 'media', 'brand', true) }}" alt=""
